...
function CounterUseEffect(){
    ...
    // 하나의 컴포넌트에서 useEffect()를 여러번 사용 가능
    // 사용법:
        // useEffect(effect함수, 의존성 배열)
    
    // effect 콜백 함수: useEffect 사용 시, 동작할 소스코드
    ...
    // 의존성 배열의 요소로 지정된 변수의 값이 변경되면 componentDidUpdate() 함수로써 useEffect가 동작함
    useEffect(() => {
    ...
    // useEffect 사용 시, effect 콜백함수의 return 부분에 콜백함수를 입력하면 해당 컴포넌트가 삭제될 때 useEffect가 동작하여 componentWillUnmount() 함수의 역할을 수행함
    useEffect(() => {
        return () => {
            console.log((`\n-------- 컴포넌트 unmount 시 동작 --------\n`))
        }
