// 자바스크립트에서는 소스코드를 별도의 파일로 나누어서 사용할 수 있음
// 기본적으로는 html 파일에서 나누어진 자바스크립트 파일을 호출하여 사용
// 모듈이라는 개념이 추가되면서 자바스크립트끼리 파일을 호출하여 사용하는 것이 가능해짐
...
// ES6에서는 import/export 가 추가되어 조금 더 쉽게 다른 자바스크립트의 소스를 가져올 수 있음
  ...
  // 확장자가 mjs인 파일은 추가 설정없이 import/export를 사용할 수 있음
...
// import: 다른 자바스크립트 모듈에서 제공하는 변수, 함수, 클래스를 가져와서 사용할 수 있음
  ...
  // named import : 다른 자바스크립트 모듈에서 export로 지정한 변수, 함수, 클래스를 가져오는 import 방식
...
// export 된 변수 가져오기
import { addr, age, email, name, phone } from "./person.mjs";
...
// export 된 함수 가져오기
import { div, multi, sub, sum } from "./person.mjs";
sum(10, 20);
sub(20, 10);
let result = multi(10);
...
console.log(`클래스 사용하기2 : ${result}`);

// 가져온 모듈에 as를 사용하여 별칭을 설정
import { sub as sub2 } from "./person2.mjs";
// person.mjs 모듈에서 제공하는 sub 함수
sub(20, 10);
// person2.mjs 모듈에서 제공하는 sub 함수
sub2(20, 10);

console.log("\n--------------\n")

// default export로 지정한 변수, 함수, 클래스 가져오기
