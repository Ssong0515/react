...
function Main() {
  ...
  return (
    <div className={'container mt-5 p-5'}>
      ...
      <button type={'button'} className={'btn btn-primary me-2'} onClick={clickBtnBoardEdit}>글수정 페이지 접속</button>
      <Link to={'/board/boardDetail'}>상세 글 보기</Link>
      <div className={'mt-3'}>
        <Link to={'/auth/login'}>로그인 페이지</Link>
        <button type={'button'} className={'btn btn-primary me-2'} onClick={clickBtnLogout}>로그 아웃</button>
      </div>
    </div>
  );
}
